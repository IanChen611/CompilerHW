...
(** {0 Library to write X86-64 programs }
    It is only a small fragment of the X86-64 assembly language.
...
type text = [ `text ] asm
  (**  of assembly code in the text section *)

type data = [ `data ] asm
  (**  of assembly code in the data section *)


type label = string
  (** the address labels are strings *)

val nop : [> ] asm
  (** the no-operation instruction. Can be found in either text or data *)
...
val print_program : Format.formatter -> program -> unit
  (** [print_program fmt p] prints the code of the program [p] into the
      formatter [fmt] *)
...
val movq: [`Q] operand -> [`Q] operand -> text
  (** be careful : all combinations of operands are not allowed *)
...
val xorq: [`Q] operand -> [`Q] operand -> text
  (** Bit manipulation operations. "and" bit by bit, "or" bit by
       bit, "not" bit by bit *)
...
val setbe: [`B] operand -> text  (* <= unsigned *)
  (** position the operand byte to 1 or 0 depending on whether the test is true or not *)

(** {2 Manipulation of the stack} *)

val pushq : [`Q] operand -> text
  (** [pushq r] place the content of [r] at the top of the stack.
      Reminder: %rsp points to the address of the last occupied cell *)

val popq : [`Q] register -> text
  (** [popq r] place the word at the top of the stack into [r] and pops the stack *)
...
val label : label -> [> ] asm
  ...
  (** a label. Can be found in text or data *)
val globl : label -> [> ] asm
  (** declaration .globl (for main, typically) *)

val comment : string -> [> ] asm
  (** place a comment in the generated code. Can be found in
      text or data *)
...
val dquad : int list -> data
  (** places a list of values on 1/2/4/8 bytes in the data area *)
val address: label list -> data
  (** places a list of addresses in the data area (with .quad) *)
val space: int -> data
  (** [space n] allocates [n] bytes (worth 0) in the data segment *)
